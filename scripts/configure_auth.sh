...
STAGE_WEBSITE_URL=$(gcloud run services describe website --region ${REGION} --project ${STAGE_PROJECT} --format "value(status.address.url)")
STAGE_CALLBACK_URL="${STAGE_WEBSITE_URL}/callback"

if [ "${PROD_PROJECT}" != "${STAGE_PROJECT}" ]; then
PROD_WEBSITE_URL=$(gcloud run services describe website --region ${REGION} --project ${PROD_PROJECT} --format "value(status.address.url)") 
...
gcloud run services update website --region ${REGION} \
