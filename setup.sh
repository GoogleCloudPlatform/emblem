...
# Import the session data bucket (if it exists)
terraform import module.application.google_storage_bucket.sessions "${STAGE_PROJECT}-sessions" || true
...
terraform import module.application.google_storage_bucket.sessions "${PROD_PROJECT}-sessions" || true
terraform apply --auto-approve 
terraform state rm module.application.google_app_engine_application.main || true
fi
